  ...
  return (
    <div className="h-screen w-full  pt-[123.99px] sm:pt-[139.98px] ">
      <div className=" absolute w-full h-[calc(100vh_-_139.98px)] -z-50">
        <img className="object-cover w-full h-full" src={sliderImage} alt="" />
      </div>
      <div className=" h-[calc(100vh_-_123.99px)] sm:h-[calc(100vh_-_139.98px)] w-full flex items-center relative ">
        <div className="flex justify-between w-full px-5">
          <button className="rounded-full h-12 w-12 md:h-14 md:w-14 lg:h-16 lg:w-16 text-base md:text-xl lg:text-2xl grid place-items-center bg-white opacity-60 hover:opacity-100 hover:text-white hover:bg-[#1367fe] border-gray-300 border-2 outline-none transition-all">
            <SlArrowLeft/>
          </button>
          <div className="flex-1 flex flex-col gap-y-3 items-start container">
            <div>
              <p className="text-lg font-medium">SUMMER 2022</p>
              <h2 className=" text-base sm:text-xl md:text-4xl lg:text-6xl font-semibold">Save up to 70%</h2>
            </div>
            <a
              href="#s"
